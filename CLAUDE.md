...
- `extension/` — Chrome Manifest V3. Popup: `popup.html/css/js` (tab-uri File, Link, Sample, Queue + iconițe History, Settings; Link implicit la prima deschidere, apoi ultimul tab sau alegerea din Settings) + `history.js`, `queue.js`, `analysis.js` (scripturi clasice încărcate înaintea lui popup.js, împart globale). `shared.js` = setări (`getSettings`), `buildFileName`, `startDownload`, istoric, helper analiză — încărcat și de popup, și de worker (`importScripts`). `background.js` (service worker: Tunebat + citirea rezultatului, Sample/badge/scurtătură) + `queue-engine.js` (coada, în worker). `offscreen.*` (înregistrarea audio, supraviețuiește popup-ului). Regulă: dacă adaugi o globală nouă, verifică să nu existe deja în celelalte scripturi din același context (nu sunt module).
...
- **Coada** (`queue-engine.js`): stare în `chrome.storage.local["queue"]`, toate modificările serializate prin `mutateQueue`; un singur element o dată; `resumeQueue()` la pornirea worker-ului repune în așteptare ce era „working". Popup-ul doar randează (`storage.onChanged`) și trimite comenzi (`queue:add/remove/retry/clear`). Ignoră `saveAs`. Keep-alive cu `getPlatformInfo` la 20s.
- **Rezultat Tunebat**: `readTunebatResult` găsește rândul după conținut (clasele CSS sunt hash-uite): elementul cu numele fișierului + cel mai apropiat strămoș care conține o celulă „X major/minor". Salvat în `analysis:<fileUrl>` (șters după 24h). Fără `\b` între celule (textul curge „…wavA minor8A120"). Copia redenumită: `/api/annotate` pentru mp3/flac, altfel doar alt nume; originalul se șterge doar dacă `settings.deleteOriginal`.
